var Coin = (function () {
  ...
  */
      ...
      addBlueprint(api, name, bp) {
        ...
        method = (data, cb) => {
          ...
          if (bp.useCustomerToken) {
            key = this.client.getCustomerToken();
          }
          return this.client.request(bp, data, key).then((res) => {
            var ref, ref1;
  ...
  BrowserClient = class BrowserClient extends Client$1 {
    constructor(opts) {
      super(opts);
    ...
    request(blueprint, opts = {}, key = this.getKey()) {
      ...
      return (new XhrPromise$1).send(opts).then((res) => {
  ...
  var blueprints, createBlueprint, i, j, len, len1, marketingModels, model, models;
  ...
  var closestGeoRate = function(grs, ctr, st, ct, pc) {
    var currentLevel, gr, i, idx, isMatch, level, retGr;
    retGr = null;
    currentLevel = -1;
    idx = -1;
    for (i in grs) {
    ...
    class Cart {
      ...
      initCart() {
        ...
        if (!cartId && (this.client.cart != null)) {
          this.client.cart.create().then((cart) => {
            var i, item, items, j, len;
            this.data.set('order.cartId', cart.id);
            items = this.data.get('order.items');
            for (i = j = 0, len = items.length; j < len; i = ++j) {
              item = items[i];
              this._cartSet(item.productId, item.quantity);
            }
            return this.onCart(cart.id);
          });
          return this.data.on('set', (name) => {
            switch (name) {
              case 'order.storeId':
                return this._cartSyncStore();
            }
          });
  ...
  // try and update item quantity
      ...
      load(id) {
        return this.client.product.get(id).then((product) => {
          ...
          for (i = j = 0, len = items.length; j < len; i = ++j) {
            ...
            if (product.id === item.id || product.slug === item.id) {
              ...
              this._cartSet(product.id, item.quantity);
              break;
            }
          }
          this.queue.shift();
      ...
      promoCode(promoCode) {
        if (promoCode != null) {
          this.invoice();
          return this.client.coupon.get(promoCode).then((coupon) => {
            if (coupon.enabled) {
              this.data.set('order.coupon', coupon);
              this.data.set('order.couponCodes', [promoCode]);
              this._cartUpdate({
                coupon: coupon,
                couponCodes: [promoCode]
              });
              if (coupon.freeProductId !== "" && coupon.freeQuantity > 0) {
                return this.client.product.get(coupon.freeProductId).then((freeProduct) => {
                  return this.invoice();
                }).catch(function(err) {
      ...
      invoice() {
        var city, country, coupon, discount, gr, i, item, items, j, k, l, len, len1, len2, len3, m, n, postalCode, quantity, rate, ref, ref1, ref2, ref3, ref4, ref5, ref6, ref7, ref8, ref9, shipping, shippingRate, shippingRates, state, subtotal, tax, taxRate, taxRates;
        ...
        taxRate = (ref4 = this.data.get('order.taxRate')) != null ? ref4 : {
          percent: 0,
          cost: 0
        };
        tax = Math.ceil(((ref5 = taxRate.percent) != null ? ref5 : 0) * subtotal + ((ref6 = taxRate.cost) != null ? ref6 : 0));
        shippingRate = (ref7 = this.data.get('order.shippingRate')) != null ? ref7 : {
          percent: 0,
          cost: 0
        };
        shipping = Math.ceil(((ref8 = shippingRate.percent) != null ? ref8 : 0) * subtotal + ((ref9 = shippingRate.cost) != null ? ref9 : 0));
      ...
      checkout(opts = {}, authOnly = false) {
        ...
        return this.client.checkout.authorize(data).then((order) => {
          ...
          if (referralProgram != null) {
            p2 = this.client.referrer.create({
              userId: this.data.get('order.userId'),
              orderId: this.data.get('order.id'),
            ...
            p = Promise$2.settle([p, p2]).then((pis) => {
              var referrer;
              order = pis[0].value;
          ...
          options = {
            orderId: this.data.get('order.id'),
            total: parseFloat(this.data.get('order.total') / 100),
            // revenue: parseFloat(order.total/100),
            shipping: parseFloat(this.data.get('order.shipping') / 100),
            tax: parseFloat(this.data.get('order.tax') / 100),
            discount: parseFloat(this.data.get('order.discount') / 100),
            coupon: this.data.get('order.couponCodes.0') || '',
  ...
  var getReferrer = function(hashReferrer = false) {
    var r, ref, referrer;
    ref = null;
  ...
  CheckoutForm = (function() {
    class CheckoutForm extends El$1$1.Form {
      init() {
        super.init(...arguments);
        this.data.on('set', (name, value) => {
          if (name === 'user.email') {
            return this.cart._cartUpdate({
              email: value,
              currency: this.data.get('order.currency'),
              mailchimp: {
                checkoutUrl: this.data.get('order.checkoutUrl')
              }
            });
          }
        });
        this.data.on('set', (name, value) => {
          if (name === 'user.name') {
            if (!this.data.get('payment.account.name')) {
              if (value) {
                this.data.set('payment.account.name', value);
              }
            }
            if (!this.data.get('order.shippingAddress.name')) {
              if (value) {
                this.data.set('order.shippingAddress.name', value);
              }
            }
          }
          return El$1$1.scheduleUpdate();
        });
        return this.data.on('set', (name, value) => {
        ...
        return this.client.account.exists(opts).then((res) => {
          ...
          return this.cart.checkout(opts).then((pRef) => {
            return pRef.p.then((order) => {
              ...
              this.mediator.trigger(Events$3.SubmitSuccess, order);
              return order;
            }).catch((err) => {
              var ref;
              if (typeof window !== "undefined" && window !== null) {
              ...
              this.mediator.trigger(Events$3.SubmitFailed, err);
              return El$1$1.scheduleUpdate();
            });
          }).catch((err) => {
            var ref;
  ...
  DepositForm = (function() {
    class DepositForm extends CheckoutForm$1 {
      init() {
  ...
  LoginForm = (function() {
    class LoginForm extends El$1$1.Form {
      init() {
        return super.init(...arguments);
      }
      ...
      _submit(event) {
        ...
        return this.client.account.login(opts).then((res) => {
          this.mediator.trigger(Events$3.LoginSuccess, res);
          return this.scheduleUpdate();
        }).catch((err) => {
          this.errorMessage = err.message;
          this.mediator.trigger(Events$3.LoginFailed, err);
          return this.scheduleUpdate();
        });
      }
  ...
  ProfileForm = (function() {
    class ProfileForm extends El$1$1.Form {
      hasOrders() {
        var orders;
      ...
      init() {
        ...
        this.client.account.get().then((res) => {
          var firstName, lastName;
          this.data.set('user', res);
          firstName = this.data.get('user.firstName');
          lastName = this.data.get('user.lastName');
          this.data.set('user.name', firstName + ' ' + lastName);
          if (this.data.get('referralProgram') && ((res.referrers == null) || res.referrers.length === 0)) {
            return requestAnimationFrame(() => {
              this.mediator.trigger(Events$3.CreateReferralProgram);
              return this.client.referrer.create({
                program: this.data.get('referralProgram'),
                programId: this.data.get('referralProgram.id'),
                userId: res.id
              }).then((res2) => {
                var refrs;
                refrs = [res2];
                this.data.set('user.referrers', refrs);
                this.mediator.trigger(Events$3.CreateReferralProgramSuccess, refrs);
                this.mediator.trigger(Events$3.ProfileLoadSuccess, res);
                return El$1$1.scheduleUpdate();
              }).catch((err) => {
                this.errorMessage = err.message;
                this.mediator.trigger(Events$3.CreateReferralProgramFailed, err);
                this.mediator.trigger(Events$3.ProfileLoadSuccess, res);
                return El$1$1.scheduleUpdate();
              });
            });
          ...
          this.errorMessage = err.message;
          this.mediator.trigger(Events$3.ProfileLoadFailed, err);
          return El$1$1.scheduleUpdate();
        });
        return super.init(...arguments);
      }
      ...
      _submit(event) {
        ...
        return this.client.account.update(opts).then((res) => {
          this.data.set('user.currentPassword', null);
          this.data.set('user.password', null);
          this.data.set('user.passwordConfirm', null);
          this.mediator.trigger(Events$3.ProfileUpdateSuccess, res);
          return this.scheduleUpdate();
        }).catch((err) => {
          this.errorMessage = err.message;
          this.mediator.trigger(Events$3.ProfileUpdateFailed, err);
          return this.scheduleUpdate();
        });
      }
  ...
  RegisterForm = (function() {
    class RegisterForm extends El$1$1.Form {
      init() {
        return super.init(...arguments);
      }
      ...
      _submit(event) {
        ...
        return this.client.account.create(opts).then((res) => {
          var latency;
          this.mediator.trigger(Events$3.RegisterSuccess, res);
          this.scheduleUpdate();
          if (this.immediateLogin && res.token) {
            this.client.setCustomerToken(res.token);
            latency = this.immediateLoginLatency / 2;
          ...
          this.errorMessage = err.message;
          this.mediator.trigger(Events$3.RegisterFailed, err);
          return this.scheduleUpdate();
        });
      }
  ...
  RegisterCompleteForm = (function() {
    class RegisterCompleteForm extends El$1$1.Form {
      init() {
        super.init(...arguments);
      ...
      _submit(event) {
        ...
        return this.client.account.enable(opts).then((res) => {
          if (res.token) {
            this.client.setCustomerToken(res.token);
          }
          this.mediator.trigger(Events$3.RegisterCompleteSuccess, res);
          return this.scheduleUpdate();
        }).catch((err) => {
          this.errorMessage = err.message;
          this.mediator.trigger(Events$3.RegisterCompleteFailed, err);
          return this.scheduleUpdate();
        });
      }
  ...
  ResetPasswordForm = (function() {
    class ResetPasswordForm extends El$1$1.Form {
      init() {
        return super.init(...arguments);
      }
      ...
      _submit(event) {
        ...
        return this.client.account.reset(opts).then((res) => {
          this.mediator.trigger(Events$3.ResetPasswordSuccess, res);
          return this.scheduleUpdate();
        }).catch((err) => {
          this.errorMessage = err.message;
          this.mediator.trigger(Events$3.ResetPasswordFailed, err);
          return this.scheduleUpdate();
        });
      }
  ...
  ResetPasswordCompleteForm = (function() {
    class ResetPasswordCompleteForm extends El$1$1.Form {
      init() {
        return super.init(...arguments);
      }
      ...
      _submit(event) {
        ...
        return this.client.account.confirm(opts).then((res) => {
          if (res.token) {
            this.client.setCustomerToken(res.token);
          }
          this.mediator.trigger(Events$3.ResetPasswordCompleteSuccess, res);
          return this.scheduleUpdate();
        }).catch((err) => {
          this.errorMessage = err.message.replace('Token', 'Link');
          this.mediator.trigger(Events$3.ResetPasswordCompleteFailed, err);
          return this.scheduleUpdate();
        });
      }
  ...
  ThankYouForm = (function() {
    class ThankYouForm extends El$1$1.Form {
      init() {
        super.init(...arguments);
        if (this.testCrypto) {
          this.test = true;
        }
        return this.on('mount', () => {
          var update;
          update = () => {
            if (this.isMetamaskInstalled()) {
              web3.eth.getAccounts((err, accounts) => {
                if (err != null) {
                  ...
                  return this.loggedIntoMetamask = false;
                } else {
                  return this.loggedIntoMetamask = true;
                }
              });
              web3.version.getNetwork((err, netId) => {
                ...
                if (netId === '1' && net === 'Mainnet') {
                  return this.metamaskNetworkMismatch = false;
                } else if (netId === '3' && net === 'Ropsten') {
                  return this.metamaskNetworkMismatch = false;
                } else {
                  return this.metamaskNetworkMismatch = true;
                }
              });
            } else {
              this.loggedIntoMetamask = false;
            }
            return this.scheduleUpdate();
          };
          update();
          this.metamaskInterval = setInterval(update, 1000);
          return this.on('unmount', () => {
            return clearInterval(this.metamaskInterval);
          });
        });
      }
      ...
      payWithMetamask() {
        ...
        try {
          return web3.eth.sendTransaction({
            ...
            El$1$1.scheduleUpdate();
            if (err) {
              this.mediator.trigger(Events$3.PayWithMetamaskFailed, err);
              this.errorMessage = err;
              return;
            }
            this.checkedOut = true;
            return this.mediator.trigger(Events$3.PayWithMetamaskSuccess, transactionHash);
          });
        } catch (error) {
          this.loading = false;
          El$1$1.scheduleUpdate();
  ...
  var View$4 = View$3 = (function() {
    class View extends El$1$1.View {
      init() {
        return super.init(...arguments);
      }
  ...
  CardCVC = (function() {
    class CardCVC extends Text$1 {
      init() {
        super.init(...arguments);
        return this.on('mount', () => {
          var el;
          el = this.root.querySelector('input');
          this._limit4 = function(e) {
            var key, value;
            key = e.keyCode;
          ...
          el.addEventListener('keypress', this._limit4);
          return this.on('unmount', () => {
            el.removeEventListener('keypress', restrictNumeric);
            return el.removeEventListener('keypress', this._limit4);
          });
        });
      }
  ...
  CardExpiry = (function() {
    class CardExpiry extends Text$1 {
      init() {
        super.init(...arguments);
        return this.on('mount', () => {
          var el;
          el = this.root.querySelector('input');
          this._limit7 = function(e) {
            var key, value;
            key = e.keyCode;
          ...
          el.addEventListener('keypress', restrictNumeric);
          el.addEventListener('keypress', this._limit7);
          return this.on('unmount', () => {
            el.removeEventListener('keypress', restrictNumeric);
            return el.removeEventListener('keypress', this._limit7);
          });
        });
      }
  ...
  CardNumber = (function() {
    class CardNumber extends Text$1 {
      init() {
        super.init(...arguments);
        return this.on('mount', () => {
          var el;
          ...
          this._identifyCard = (e) => {
            ...
            if (this.cardType) {
              this.root.classList.remove(this.cardType);
            }
            this.root.classList.remove('identified');
            this.root.classList.remove('unknown');
            value = el.value + String.fromCharCode(key);
            ...
            if (card) {
              ...
              if (this.cardType) {
                this.root.classList.add(this.cardType);
                this.root.classList.add('identified');
              } else {
                this.root.classList.add('unknown');
              }
            }
          ...
          el.addEventListener('keypress', restrictNumeric);
          el.addEventListener('keypress', this._identifyCard);
          return this.on('unmount', () => {
            el.removeEventListener('keypress', restrictNumeric);
            return el.removeEventListener('keypress', this._identifyCard);
          });
        });
      }
  ...
  var QuantitySelect$1 = QuantitySelect = (function() {
    class QuantitySelect extends Select$1 {
      options() {
        return opts;
      }
  ...
  var ShippingAddressCity$1 = ShippingAddressCity = (function() {
    class ShippingAddressCity extends Text$1 {
      init() {
        return super.init(...arguments);
      }
  ...
  var ShippingAddressLine1$1 = ShippingAddressLine1 = (function() {
    class ShippingAddressLine1 extends Text$1 {
      init() {
        return super.init(...arguments);
      }
  ...
  var ShippingAddressLine2$1 = ShippingAddressLine2 = (function() {
    class ShippingAddressLine2 extends Text$1 {
      init() {
        return super.init(...arguments);
      }
  ...
  var ShippingAddressName$1 = ShippingAddressName = (function() {
    class ShippingAddressName extends Text$1 {
      init() {
        return super.init(...arguments);
      }
  ...
  var ShippingAddressPostalCode$1 = ShippingAddressPostalCode = (function() {
    class ShippingAddressPostalCode extends Text$1 {
      init() {
        return super.init(...arguments);
      }
  ...
  var CheckBox$1 = CheckBox = (function() {
  ...
  Terms = (function() {
    class Terms extends CheckBox$1 {
      init() {
        return super.init(...arguments);
      }
  ...
  var UserCurrentPassword$1 = UserCurrentPassword = (function() {
    class UserCurrentPassword extends Text$1 {
      init() {
        return super.init(...arguments);
      }
  ...
  var UserEmail$1 = UserEmail = (function() {
    class UserEmail extends Text$1 {
      init() {
        return super.init(...arguments);
      }
  ...
  var UserName$1 = UserName = (function() {
    class UserName extends Text$1 {
      init() {
        return super.init(...arguments);
      }
  ...
  var UserPasswordConfirm$1 = UserPasswordConfirm = (function() {
    class UserPasswordConfirm extends Text$1 {
      init() {
        return super.init(...arguments);
      }
  ...
  var UserPassword$1 = UserPassword = (function() {
    class UserPassword extends Text$1 {
      init() {
        return super.init(...arguments);
      }
  ...
  UserUsername = (function() {
    class UserUsername extends Text$1 {
      init() {
        return super.init(...arguments);
      }
  ...
  GiftEmail = (function() {
    class GiftEmail extends Text$1 {
      init() {
        return super.init(...arguments);
      }
  ...
  GiftMessage = (function() {
    class GiftMessage extends TextBox$1 {
      init() {
        return super.init(...arguments);
      }
  ...
  GiftToggle = (function() {
    class GiftToggle extends CheckBox$1 {
      init() {
        return super.init(...arguments);
      }
  ...
  GiftType = (function() {
    class GiftType extends Select$1 {
      init() {
        return super.init(...arguments);
      }
